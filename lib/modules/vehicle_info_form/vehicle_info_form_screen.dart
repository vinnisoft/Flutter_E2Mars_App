...
class VehicleInfoFormScreen extends GetView<VehicleInfoFormController> {
   VehicleInfoFormScreen({super.key});
      ...
      body: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            ).paddingOnly(bottom: 6.w,left: 22.w),
            CustomDropdown(
              dropdownItems: ["Car", "Bike", "Truck"],
              onChanged: (value){
              ...
              onUpload: () async {
                ...
                showImagePickerBottomSheet(()async{
                  if (await Helper.storagePermission()){
                    ...
                    if (file != null) controller.pickedFile.value = file;
                  }
                },
                        ()async{
                      if (await Helper.storagePermission()){
                ...
                // bool permissionGranted =  await Helper.storagePermission();
                //
  ...
  _step1Form() => Form(
      ...
      child: Column(
        children: [
               ...
               TextFieldWidget(
                  textController: controller.bandModelTextController,
                  // focusNode: controller.firstNameFocusNode,
                  label: "Brand & Model".tr,
                  validate: (value) => FieldChecker.fieldChecker(
                      value: value ?? '', message: "Brand & Model"),
                ).paddingSymmetric(vertical: 6.h),

                 TextFieldWidget(
                  textController: controller.plateNumberTextController,
                  // focusNode: controller.lastNameFocusNode,
                  label: "Plate Number".tr,
                  validate: (value) => FieldChecker.fieldChecker(
                      value: value ?? '', message:"Plate Number"),
                ).paddingSymmetric(vertical: 6.h),


          TextFieldWidget(
            textController: controller.manufactureTextController,
            // focusNode: controller.emailFocusNode,
            label: "Year of manufacture",
            validate: (value) => FieldChecker.fieldChecker(
                value: value ?? '', message: "Year of manufacture"),
          ).paddingSymmetric(vertical: 6.h),

          TextFieldWidget(
            textController: controller.colorTextController,
            // focusNode: controller.emailFocusNode,
            label: "Color",
            validate: (value) => FieldChecker.fieldChecker(
                value: value ?? '', message: "Color"),
          ).paddingSymmetric(vertical: 6.h),
...
class UploadImageBox2 extends StatelessWidget {
  final String label;
  final VoidCallback onUpload;
  final String? imagePath; // 👈 Add this

  const UploadImageBox2({
    Key? key,
    required this.label,
